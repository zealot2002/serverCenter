...
public class ClassUtils {
    ...
    private static SharedPreferences getMultiDexPreferences(Context context) {
        return context.getSharedPreferences("multidex.version", VERSION.SDK_INT < 11?0:Context.MODE_MULTI_PROCESS);
    }
        ...
        try {
            ...
            while(var5.hasNext()) {
                ...
                if(clazz.isAssignableFrom(aClass) && !clazz.equals(aClass) && !aClass.isInterface()) {
                    objectList.add(Class.forName(className).getConstructor(new Class[0]).newInstance(new Object[0]));
                }
            }
    ...
    private static List<String> tryLoadInstantRunDexFile(ApplicationInfo applicationInfo) {
        ...
        if(VERSION.SDK_INT >= 21 && null != applicationInfo.splitSourceDirs) {
            instantRunSourcePaths.addAll(Arrays.asList(applicationInfo.splitSourceDirs));
            Log.d("ClassUtils", "Found InstantRun support");
        } else {
            try {
                Class pathsByInstantRun = Class.forName("com.android.tools.fd.runtime.Paths");
                Method getDexFileDirectory = pathsByInstantRun.getMethod("getDexFileDirectory", new Class[]{String.class});
